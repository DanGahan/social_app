...
def populate_data():
    ...
    # Clear existing data (optional, but good for fresh test data)
    ...
    for i in range(1, 21):
        ...
        display_name = f"{random.choice(FIRST_NAMES)} {random.choice(LAST_NAMES)}" # Generate random display name
        profile_picture_url = random.choice(PROFILE_PIC_URLS) # Assign random profile pic

        user = User(email=email, password_hash=hashed_password, display_name=display_name, profile_picture_url=profile_picture_url)
        session.add(user)
        users.append(user)
    session.commit()
    ...
    for user in users:
        ...
        for i in range(num_posts):
            image_url = random.choice(IMAGE_URLS) # Use random image URL
            caption = random.choice(CAPTION_MESSAGES) # Assign random message to caption
    ...
    for i, user1 in enumerate(users):
        # Connect each user to a couple of other random users
        possible_connections = [u for u in users if u.id != user1.id]
        random.shuffle(possible_connections)

        for user2 in possible_connections[:2]: # Try to connect to 2 other users
            # Ensure user_id1 < user_id2 for unique constraint
