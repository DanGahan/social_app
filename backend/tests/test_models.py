...
def test_user_creation(setup_database):
    ...
    user = User(email='test@example.com', password_hash='hashed_password', display_name='Test User')
    session.add(user)
    session.commit()
...
def test_post_creation(setup_database):
    ...
    post = Post(user_id=user.id, image_url='http://example.com/image.jpg', caption='Test Post')
    session.add(post)
    session.commit()
...
def test_connection_request_creation(setup_database):
    session = setup_database
    from_user = User(email='from@example.com', password_hash='hashed_password_from')
    to_user = User(email='to@example.com', password_hash='hashed_password_to')
    session.add_all([from_user, to_user])
    session.commit()

    request = ConnectionRequest(from_user_id=from_user.id, to_user_id=to_user.id, status='pending')
    session.add(request)
    session.commit()
