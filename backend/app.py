...
def search_users(current_user):
    ...
    connections = session.query(Connection).filter(
        or_(Connection.user_id1 == current_user.id, Connection.user_id2 == current_user.id)
    ).all()
    connected_user_ids = []
    ...
    pending_requests = session.query(ConnectionRequest).filter(
        or_(
...
def request_connection(current_user):
    ...
    app.logger.debug(f"DEBUG: Requesting connection from {current_user.id} to {to_user_id}")

    if not to_user_id:
        return jsonify({'message': 'to_user_id is required'}), 400

    if current_user.id == to_user_id:
        return jsonify({'message': 'Cannot send connection request to yourself'}), 400
    ...
    if existing_connection:
        app.logger.debug(f"DEBUG: Existing connection found: user1={existing_connection.user_id1}, user2={existing_connection.user_id2}")
        return jsonify({'message': 'Already connected with this user'}), 409

    new_request = ConnectionRequest(from_user_id=current_user.id, to_user_id=to_user_id, status='pending')
    try:
        session.add(new_request)
        session.commit()
        return jsonify({'message': 'Connection request sent successfully', 'request_id': new_request.id}), 201
    except IntegrityError:
        session.rollback()
        return jsonify({'message': 'Connection request already exists or a similar issue occurred.'}), 409
    except Exception as e:
        session.rollback()
        app.logger.error(f"Error sending connection request: {e}")
        return jsonify({'message': 'An error occurred while sending the connection request.'}), 500
...
def accept_connection(current_user):
    ...
    connection_request = session.query(ConnectionRequest).filter_by(id=request_id, to_user_id=current_user.id, status='pending').first()

    if not connection_request:
        return jsonify({'message': 'Pending connection request not found or not for this user'}), 404
    ...
    return jsonify({'message': 'Connection accepted successfully', 'connection_id': new_connection.id}), 200
...
def deny_connection_request(current_user):
    ...
    try:
        ...
        session.commit()
        return jsonify({'message': 'Connection request denied successfully'}), 200
    except Exception as e:
        session.rollback()
        app.logger.error(f"Error denying connection request: {e}")
        return jsonify({'message': 'An error occurred while denying the connection request.'}), 500

@app.route('/users/<int:user_id>/connections', methods=['GET'])
@token_required
def get_user_connections(current_user, user_id):
    if current_user.id != user_id:
        return jsonify({'message': 'Cannot access other user\'s connections!'}), 403
...
def get_pending_requests(current_user, user_id):
    if current_user.id != user_id:
        return jsonify({'message': 'Cannot access other user\'s pending requests!'}), 403

    pending_requests = session.query(ConnectionRequest).filter_by(to_user_id=user_id, status='pending').all()
...
def get_sent_requests(current_user, user_id):
    if current_user.id != user_id:
        return jsonify({'message': 'Cannot access other user\'s sent requests!'}), 403

    sent_requests = session.query(ConnectionRequest).filter_by(from_user_id=user_id, status='pending').all()
...
def get_connections_posts(current_user, user_id):
    if current_user.id != user_id:
        return jsonify({'message': 'Cannot access other user\'s connections\' posts!'}), 403
    ...
    posts = session.query(Post).filter(Post.user_id.in_(connected_user_ids)).order_by(Post.created_at.desc()).all()
