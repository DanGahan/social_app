...
class CoreViewsTest(TestCase):
    ...
    def test_register_view_success(self, mock_post):
        ...
        response = self.client.post(self.register_url, {'email': 'test@example.com', 'password': 'password123', 'confirm_password': 'password123'})
    ...
    def test_register_view_invalid_form(self, mock_post):
        response = self.client.post(self.register_url, {'email': 'invalid-email', 'password': '123', 'confirm_password': '456'})
    ...
    def test_login_view_invalid_response_from_backend(self, mock_post):
        ...
        messages_list = list(get_messages(response.wsgi_request))
        self.assertEqual(len(messages_list), 1)
        self.assertEqual(str(messages_list[0]), 'Login failed: Invalid response from server.')
